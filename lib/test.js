...
Test.prototype.submitForm = function (form, params, cb) {
    ...
    var opts = url.parse(form.action || pwin.location.href.split('?')[0]);
    opts.path = opts.pathname + (opts.search || '');
    opts.method = (form.method || 'GET').toUpperCase();
    ...
    var req = http.request(opts, function (res) {
        res.on('data', function (buf) {
            doc.body.innerHTML += buf.toString();
        });
